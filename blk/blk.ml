...
let mapper which =
  ...
  let sector_size = Mkernel.Block.sector_size t.blk in
  if len <> sector_size
  then Fmt.invalid_arg "Sc.mapper: [len] (%d, sector:%d) is not sector-aligned" len sector_size;
  let off, max = bounds t which in
  let abs_off = off + rel_off in
  if abs_off < 0 || rel_off < 0 || len < 0 || rel_off > max - len
  then Fmt.invalid_arg "Sc.mapper: out of bounds ([%d:%d], slot %d, %a)" abs_off len
         (Int64.to_int t.generation land 1) pp_zone (zone t which);
  let bstr = Bstr.create len in
  Mkernel.Block.atomic_read t.blk ~src_off:abs_off bstr;
  bstr
...
let writev which =
  ...
  then Fmt.invalid_arg "Sc.writer: [pos] (%d, sector:%d) is not sector-aligned" rel_off sector_size;
  if len > max - rel_off
  then Fmt.invalid_arg "Sc.writer: out of bounds ([%d:%d], %a)" (off + rel_off) len
         pp_zone (zone t which);
  let write dst_off bstr =
    if Bstr.length bstr <> sector_size
    then Fmt.invalid_arg "Sc.writer: page (%d, sector:%d) is not sector-aligned"
           (Bstr.length bstr) sector_size;
    Mkernel.Block.atomic_write t.blk ~dst_off bstr;
...
let source t which =
  ...
  let init () = 0 in
  let pull rel_off =
    if rel_off >= max then None
    else begin
      let bstr = Bstr.create sector_size in
      Mkernel.Block.atomic_read t.blk ~src_off:(off + rel_off) bstr;
      Some (bstr, rel_off + sector_size)
    end in
  let stop _ = () in
...
let write blk t =
  ...
  Mkernel.Block.atomic_write blk ~dst_off:(slot * sector_size) bstr
...
let make ~wr ~rd blk =
  ...
  let bstr = Bstr.create sector_size in
  let slot n =
    Mkernel.Block.atomic_read blk ~src_off:(n * sector_size) bstr;
...
let format ?(ratio= 1. /. 3.) ~rd ~wr ?length blk =
  ...
  let length = match length with
    ...
    | None -> Mkernel.Block.length blk in
  if ratio <= 0. || ratio >= 1. then invalid_arg "Sc.format: bad ratio";
